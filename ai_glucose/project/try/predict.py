...
def _select_model_family(models: Dict[str, Any], req: Dict[str, Any]) -> Tuple[Dict[str, Any], Any]:
    if not isinstance(models, dict) or "segment_models" not in models:
        graph_models = models
        final_model_120 = None
        if isinstance(models, dict) and "graph_models" in models:
            graph_models = models.get("graph_models", {})
            final_model_120 = models.get("final_model_120")
        return graph_models, final_model_120
    ...
    return segment_bundle.get("graph_models", {}), segment_bundle.get("final_model_120")
...
def apply_meal_guardrails(
    ...
    # 1) Prevent late rebound (90~120m) after decline started.
    ...
    for i in range(1, len(out)):
        ...
        if off >= 90 and decline_started and cur > prev + 1.0:
            out[i]["predicted_glucose"] = round(prev + 1.0, 3)

    # 2) For meal without bolus, avoid unrealistic deep drop below current.
...
def predict_glucose(req: Dict[str, Any]) -> Dict[str, Any]:
    ...
    graph_models, final_model_120 = _select_model_family(models=models, req=req)
    ...
    # final 120 override (optional)
    if USE_FINAL_120_OVERRIDE and final_model_120 is not None:
        final_pred_delta = None
        if isinstance(final_model_120, dict) and "base" in final_model_120:
            final_pred_delta = _predict_delta_with_model_entry(final_model_120, X, req)
        elif hasattr(final_model_120, "predict"):
            final_pred_delta = float(final_model_120.predict(X)[0])

        if final_pred_delta is not None:
            final_pred_glucose = _clamp_glucose(current_glucose + final_pred_delta)
            last_120 = next((p for p in forecast if p["offset_minutes"] == 120), None)
            if last_120 is not None:
                last_120["predicted_glucose"] = round(float(final_pred_glucose), 3)
                # Keep 120m override continuous with 110m point.
                idx_120 = next((i for i, p in enumerate(forecast) if p["offset_minutes"] == 120), None)
                if idx_120 is not None and idx_120 > 0:
                    prev_val = float(forecast[idx_120 - 1]["predicted_glucose"])
                    cur_val = float(forecast[idx_120]["predicted_glucose"])
                    bounded = float(np.clip(cur_val, prev_val - 12.0, prev_val + 12.0))
                    forecast[idx_120]["predicted_glucose"] = round(bounded, 3)
    ...
    if bool(req.get("debug", False)):
        ...
        result["debug"] = {
            ...
            "final_model_120_applied": bool(USE_FINAL_120_OVERRIDE and final_model_120 is not None),
        }
